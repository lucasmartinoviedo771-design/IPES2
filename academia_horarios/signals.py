...
def evitar_solape_docente(_sender, instance: HorarioClase, action, pk_set, **kwargs):
    ...
    conflictos = (
        HorarioClase.objects.filter(timeslot_id=timeslot_id, comision__periodo_id=periodo_id)
    ...
    if conflictos.exists():
        ...
        nombres = list(
            Docente.objects.filter(id__in=pk_set).values_list("apellido_nombre", flat=True)
        )
